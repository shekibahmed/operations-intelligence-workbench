...
- **No `tours/` in this pack — the guided tour lives in app code, and it
  ...
  `apps/web/src/lib/tour/steps.ts` carries an `asset-reliability` step
  list (generalised to one list per pack under OIW-702), scripted in
  `docs/DEMO_SCRIPT.md` ("Full Guided Demonstration — Asset
  Reliability") and exercised by `apps/web/e2e/north-star.spec.ts`. Tour
  steps are implemented per pack in the web app rather than as `tours/`
  manifest content, so `manifest.yaml` intentionally omits the `tours`
  field. That field is optional in `@oiw/contracts`
